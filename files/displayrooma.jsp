...
<td>
  <label id="roomcount<%=t%>"><%=rs.getString("roomcount")%></label></td>
<td>	
  <label id="examecount<%=t%>"><%=rs.getString("examecount")%></label>
  </td>
  <td>
    <label id= "roomtype<%=t%>"><%=rs.getString("roomtypename")%></label>
</td>    

     
                
				
<td align="center" valign="middle"><input align="center"  type="button" class="btn btn-green big" value="无" /></td>

</tr>
<%}%>
</table>
<p>
<%if(pageNow!=1){%><input align="center"  type="button" class="btn btn-green big" value="上一页" onclick="query(9,<%=(pageNow-1)%>,'<%=UserName%>')"/><%}%>
<%for(int i=1;i<=pageCount;i++){%><input align="center"  type="button" class="btn btn-green big" value="<%=i%>" onclick="query(9,<%=i%>,'<%=UserName%>')"/><%}%>
<%if(pageNow!=pageCount){%><input align="center"  type="button" class="btn btn-green big" value="下一页" onclick="query(9,<%=(pageNow+1)%>,'<%=UserName%>')"/><%}%>
</p></td>
  </tr>  
  </table>
<%}
